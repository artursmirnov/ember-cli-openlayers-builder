...
module.exports = {
  ...
  normalizeConfig: function(config) {
    ...
    if (config.compile) {
      ...
      config.compile.compilation_level = "WHITESPACE_ONLY";

      // prettify output
      config.compile.formatting = "PRETTY_PRINT";
    }
    ...
    return new RSVP.Promise(function(resolve, reject) {
      buildOpenlayers(config, function(error, compiledLib) {
        if (error) {
          ...
          compiledLib = compiledLib.replace('module.exports = factory()', 'exports.default = factory.apply(window)');
